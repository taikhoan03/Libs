...
{   
    ...
    public sealed class ExecuteReaderCommand : CodeActivity
    {
        ...
        public InArgument<NpgsqlConnection> ConnectionInput { get; set; }
        public InArgument<bool> IsCloseAfterExecute { get; set; }
        public InArgument<CommandType> Command_Type { get; set; }
        public InArgument<NpgsqlParameter[]> Parameters { get; set; }
        ...
        protected override void Execute(CodeActivityContext context)
        {
            ...
            var conn = context.GetValue(this.ConnectionInput);
            var IsClose=context.GetValue(this.IsCloseAfterExecute);
            var Command_Type = context.GetValue(this.Command_Type);
            var parameters= context.GetValue(this.Parameters);
            var timeout= context.GetValue(this.Timeout);
                ...
                while (dr.Read())
                {
                    ...
                    for (fieldIndex = 0; fieldIndex < fieldCount; fieldIndex++)
                    {
                        for (int icount = 0; icount < propertyFilterCount; icount++)
                        {
                            ...
                            {
                                //propertiesFilter[icount].SetValue(instanceOfClass, 
                                //    Convert.ChangeType(dr[fieldIndex].ToString(), properties[icount].PropertyType), null);
                                propertiesFilter[icount].SetValue(instanceOfClass,
                                    ChangeType<object>(dr[fieldIndex]), null);
                            }
                        }
    ...
    //        NpgsqlCommand command = new NpgsqlCommand(SqlCommandString);
    //        NpgsqlDataReader dr;
    //        List<T> list=null;
    //        try
    //        {
