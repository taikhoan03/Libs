...
{   
    ...
    public sealed class ExecuteReaderCommand : CodeActivity
    {
        ...
        protected override void Execute(CodeActivityContext context)
        {
            ...
            int fieldIndex = 0; int _icount = 0;
            try
            {
                while (dr.Read())
                {
                    ...
                    {
                        for (_icount = 0; _icount < propertyFilterCount; _icount++)
                        {
                            var drname = dr.GetName(fieldIndex);
                            if (propertiesFilter[_icount].Name.ToLower() == drname.ToLower() && !string.IsNullOrEmpty(dr[fieldIndex].ToString()))
                                propertiesFilter[_icount].SetValue(instanceOfClass, Convert.ChangeType(dr[fieldIndex].ToString(), properties[_icount].PropertyType), null);
                        }
