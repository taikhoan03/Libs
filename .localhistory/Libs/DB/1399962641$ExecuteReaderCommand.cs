        ...
        protected override void Execute(CodeActivityContext context)
        {
            ...
            try
            {
                while (dr.Read())
                {
                    ...
                    var instanceOfClass=Activator.CreateInstance<context.GetValue(this.InputType).GetType()>();
                    if (type.GetConstructor(Type.EmptyTypes) == null)
                    {
                        context.SetValue(this.ListRecords, Convert.ChangeType(dr[0].ToString(), instanceOfClass.GetType()));
                        return;
                    }
